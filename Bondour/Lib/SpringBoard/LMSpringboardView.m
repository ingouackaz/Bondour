...
- (void)centerOnIndex:(NSUInteger)index zoomScale:(CGFloat)zoomScale animated:(BOOL)animated
{
 ...
 NSLog(@"count itemViews %lu", (unsigned long)_itemViews.count);
...
- (void)layoutSubviews
{
  ...
  if(_minimumZoomLevelIsDirty == YES)
  {
    ...
    {
      [self centerOnIndex:_lastFocusedViewIndex zoomScale:_zoomScaleCache animated:YES];
    }
